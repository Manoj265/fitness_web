...
def resetpassword(request):
    ...
    query="update user set password='"+a+"' where email='"+b+"'"
...
def newpassword(request):
    ...
    x=False
    for i in result:
        if i[0]==request.POST["text2"]:
                x=True
                break
    if x==True:
        d={"email":request.POST["text2"],"mobilenumber":request.POST["text3"]}
        return render(request,"newpassword.html",{"ar":d})
    else:
        return HttpResponse("invalid otp")

def enterotp(request):
    d={"email":request.GET["email"],"mobilenumber":request.GET["mobilenumber"]}
    return render(request,"enterotp.html",{"ar":d})
@csrf_exempt
def generateotp(request):
    ...
    x=False
    for i in result:
        if str(i[0])==request.POST["text1"] and str(i[2])==request.POST["text2"]:
            x=True
            break
    if x==True:
        a=request.POST["text1"]
        otp=randint(1000,9999)
        s="update user set Otp="+str(otp)+" where email='"+a+"'"
        result=Fetchall(s)
        return HttpResponseRedirect("enterotp?email="+request.POST["text1"]+"&mobilenumber="+request.POST["text2"])
    else:
